...
class VQAModel(tf.keras.Model):
    ...
    def train(self, train_captions, train_image_features, padding_index, batch_size=30):
        ...
        for index, end in enumerate(range(batch_size, len(train_captions)+1, batch_size)):
            ...
            # TODO LATER ADD <START> AND <STOP> TOKENS
            ...
            # TODO have to pass in the question too
            ...
            ## NEW Perform a backward pass.
            with tf.GradientTape() as tape: 
                logits = self.call(batch_image_features, decoder_input)          
                loss = self.loss_function(logits, decoder_labels, mask)  
            
            train_acc = self.accuracy_function(logits, decoder_labels, mask)
            #print(f"Accuracy on training set after {index} training steps: {train_acc}")
