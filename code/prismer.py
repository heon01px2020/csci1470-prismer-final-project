...
from model import VQAModel, accuracy_function, loss_function
...
def main(args):
    ...
    train_questions  = np.array(data_dict['train_questions'])
    test_questions   = np.array(data_dict['test_questions'])
    train_img_feats = feat_prep(data_dict['train_image_features'])
    test_img_feats  = feat_prep(data_dict['test_image_features'])
    train_answers   = np.array(data_dict['train_answers']) #list of lists of strings
    print(f"train_questions: {train_questions}")
    print(f"train_answers: {train_answers}")
    test_answers    = np.array(data_dict['test_answers'])
    ...
    if args.task in ('train', 'both'):
        ...
        model = VQAModel(decoder)
        
        compile_model(model, args)
        train_model(
            model, train_questions, train_img_feats, word2idx['<pad>'], args, 
            valid = (train_answers, test_img_feats) # TODO
        )
        if args.chkpt_path: 
    ...
    if args.task in ('test', 'both'):
        ...
        if not (args.task == 'both' and args.check_valid):
            test_model(model, test_questions, test_img_feats, word2idx['<pad>'], args)
...
def load_model(args):
    ...
    model.test    = partial(VQAModel.test,    model)
    model.train   = partial(VQAModel.train,   model)
    model.compile = partial(VQAModel.compile, model)
    compile_model(model, args)
